...
func (a *CarrionAnalyzer) GetCompletions(uri lsp.DocumentURI, position lsp.Position) []lsp.CompletionItem {
	doc := a.documentStore.GetDocument(uri)
	if doc == nil {
...
func (a *CarrionAnalyzer) getSelfCompletions(uri lsp.DocumentURI, position lsp.Position) []lsp.CompletionItem {
...
func (a *CarrionAnalyzer) getObjectCompletions(uri lsp.DocumentURI, objectName string) []lsp.CompletionItem {
...
func (a *CarrionAnalyzer) getLocalCompletions(uri lsp.DocumentURI, position lsp.Position) []lsp.CompletionItem {
	...
	for _, symbol := range locals {
		...
		completions = append(completions, lsp.CompletionItem{
			Label:  symbol.Name,
			Kind:   kind,
			Detail: symbol.Type + (symbol.SpellbookName != "" ? " of " + symbol.SpellbookName : ""),
...
func (a *CarrionAnalyzer) getGlobalCompletions() []lsp.CompletionItem {
	...
	for _, symbol := range globals {
		...
		switch symbol.Type {
		case "spellbook":
			kind = protocol.CompletionItemKind["spellbook"]
		case "spell":
			kind = protocol.CompletionItemKind["spell"]
		case "variable":
			kind = protocol.CompletionItemKind["variable"]
		case "const":
			kind = protocol.CompletionItemKind["const"]
		default:
			kind = protocol.CompletionItemKind["variable"]
		}
...
func (a *CarrionAnalyzer) FindDefinition(uri lsp.DocumentURI, position lsp.Position) []lsp.Location {
	doc := a.documentStore.GetDocument(uri)
	if doc == nil {
...
func (a *CarrionAnalyzer) GetHoverInfo(uri lsp.DocumentURI, position lsp.Position) *lsp.Hover {
	...
	if isCarrionKeyword(symbolName) {
		return &lsp.Hover{
			Contents: lsp.MarkupContent{
				...
				Value: formatKeywordDocumentation(symbolName),
			},
			Range: &symbolRange,
		}
	}
	...
	case "variable":
		content = fmt.Sprintf("**variable** %s: %s\n\n%s", symbol.Name, symbol.ValueType, symbol.Documentation)
	case "field":
		content = fmt.Sprintf("**field** %s of %s\n\n%s", symbol.Name, symbol.SpellbookName, symbol.Documentation)
	case "method":
		var params string
		if symbol.Parameters != nil {
			paramStrs := make([]string, 0, len(symbol.Parameters))
		...
		content = fmt.Sprintf("**method** %s.%s(%s)\n\n%s", symbol.SpellbookName, symbol.Name, params, symbol.Documentation)
	default:
